...
@picthon.on(events.NewMessage(outgoing=True, pattern=r"\.الصيد"))
async def _(event):
    await event.edit(
        '''
**
...
@picthon.on(events.NewMessage(outgoing=True, pattern=r"\.الانواع"))
async def _(event):
    if ispay2[0] == "yes":
...
@picthon.on(events.NewMessage(outgoing=True, pattern=r"\.صيد"))
...
@picthon.on(events.NewMessage(outgoing=True, pattern=r"\.تثبيت"))
async def _(event):
    msg = event.text.split()
    try:
        ch = str(msg[2])
    ...
    isclaim.clear()
    isclaim.append("off")
    await picthon.send_message(event.chat_id, "**- تم الانتهاء من التثبيت بنجاح**")


@picthon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة الصيد"))
async def _(event):
    if "on" in isclaim:
...
@picthon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة التثبيت"))
async def _(event):
    if "on" in isauto:
        ...
        await event.edit("-لقد حدث خطأ ما وتوقف الامر لديك")
@picthon.on(events.NewMessage(outgoing=True, pattern=r"\.ت (.*)"))
async def _(event):
    if ispay2[0] == "yes":
        trys = 0
        ...
        if msg[0] == "تلقائي":  # تثبيت تلقائي عدد يوزر قناة
            ...
            @picthon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة ت "))
            async def _(event):
                if "on" in isauto:
            ...
            for i in range(int(msg[0])):
                ...
                if "Available" in isav:
                    try:
                        await picthon(functions.channels.UpdateUsernameRequest(
                            channel=ch, username=username))
...
⎱ by : @a_t_9 **
    ''')
                        ...
                        await picthon.send_message(event.chat_id, f'''خطأ مع {username}
    الخطأ :
    {str(eee)}''')
            ...
            isclaim.clear()
            isclaim.append("off")
            await picthon.send_message(event.chat_id, "تم الانتهاء من التثبيت التلقائي")
